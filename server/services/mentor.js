...
mentorRouter.post('/addNewTeacher', upload2.single('avtImage'), generateTeacherId, async (req, res) => {
    const { hoten, hocham, hocvi, mail, tennh, stknh, cccd, gioitinh, ngaysinh, khoaid, sdt, giangvienid } = req.body;
    ...
    try {
        ...
        const result = await addNewTeacher(
            ...
            imageUrl // Truyền null nếu không có ảnh
        );
...
mentorRouter.put('/updateTeacher/:id', upload2.single('avtImage'), async (req, res) => {
    const { id } = req.params;
